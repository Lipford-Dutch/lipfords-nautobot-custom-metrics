...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.tns_custom_metrics.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.tns_custom_metrics.compose_http_timeout,
        "NAUTOBOT_VER": context.tns_custom_metrics.nautobot_ver,
        "PYTHON_VER": context.tns_custom_metrics.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.tns_custom_metrics.project_name}",
        f'--project-directory "{context.tns_custom_metrics.compose_dir}"',
    ]

    for compose_file in context.tns_custom_metrics.compose_files:
        compose_file_path = os.path.join(context.tns_custom_metrics.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.tns_custom_metrics.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.tns_custom_metrics.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.tns_custom_metrics.python_ver
    dockerfile_path = os.path.join(context.tns_custom_metrics.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.tns_custom_metrics.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.tns_custom_metrics.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.tns_custom_metrics.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations tns_custom_metrics"
...
def pylint(context):
    ...
    command = f"{base_pylint_command} tns_custom_metrics"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("tns_custom_metrics") / Path("migrations")
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'tns_custom_metrics/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.tns_custom_metrics.local):
